...
const AssessDrawer = ({ isOpen, onClose, returnRequest, bookData, onSave }) => {
    ...
    const bookPriceOriginal = Math.round(bookPriceUSD * 25000); // Original price in VND
    
    // Get copy condition to calculate current value
    const copyCondition = returnRequest?.borrowedCondition || 100;
    const bookPriceCurrent = Math.round((bookPriceOriginal * copyCondition) / 100); // Current value based on condition
    ...
    // Calculate overdue based on CURRENT VALUE (adjusted by condition)
    ...
    const addDamageLine = () => {
        setDamageLines([...damageLines, {
            id: Date.now(),
            level: 'MODERATE',
            percentage: 30, // % of CURRENT book value (adjusted by condition)
            note: '',
            subtotal: Math.round((30 * bookPriceCurrent) / 100)
        }]);
    };
    ...
    const updateDamageLine = (id, field, value) => {
        setDamageLines(damageLines.map(line => {
            if (line.id === id) {
                ...
                // Recalculate subtotal based on percentage of CURRENT value
                if (field === 'percentage') {
                    updated.subtotal = Math.round((value * bookPriceCurrent) / 100);
                }
                return updated;
            }
            return line;
                        ...
                        <div className="bg-blue-50 rounded-xl p-4 space-y-2">
                            ...
                            <div className="border-t border-blue-200 pt-2 mt-2">
                                ...
                                <p className="text-xs text-gray-500 mt-1 italic">
                                    = {bookPriceOriginal.toLocaleString()}đ × {copyCondition}%
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3 text-xs">
                            ...
                            <div className="bg-white rounded-lg p-3">
                                ...
                                <div className="space-y-1 text-gray-700">
                                    <p><span className="font-medium">Formula:</span> Days late × Rate% × Current value</p>
                                    <p><span className="font-medium">Rate:</span> 1-5% of current value per day</p>
                                    <p><span className="font-medium">Current value:</span> {bookPriceCurrent.toLocaleString()} đ (= Original × Condition%)</p>
                                    <p className="text-gray-600 italic">Example: 3 days × 1% × 120,000đ = 3,600đ</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                {damageLines.map(line => (
                                    <div key={line.id} className="bg-gray-50 rounded-xl p-4 space-y-3">
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                <label className="text-xs text-gray-600 block mb-1">Book current value (đ)</label>
                                                <input
                                                    type="text"
                                                    value={bookPriceCurrent.toLocaleString()}
                                                    disabled
