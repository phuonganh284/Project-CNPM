...
import { getCopiesByBook, addCopy, deleteCopy } from "../../services/copyService";
...
const BookCopiesPage = () => {
    ...
    const loadCopies = async () => {
        try {
            setLoading(true);
            const data = await getCopiesByBook(bookId);
            setCopies(data);
        } catch (err) {
    ...
    return (
        <div className="p-10">
            <button onClick={() => navigate(-1)} className="mb-4 rounded hover:underline cursor-pointer">
                ← Back
            </button>
            ...
            {copies.length === 0 ? (
                ...
                <table className="w-full border border-gray-300 rounded bg-white shadow-sm">
                    <thead>
